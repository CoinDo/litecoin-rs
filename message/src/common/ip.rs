...
use serialization::{Deserializable, Error as ReaderError, Reader, Serializable, Stream};
use std::{io, net, str};
...
mod test {
    ...
    fn test_ip_deserialize() {
        let ip: IpAddress = deserialize(&[
            0x00u8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x0a, 0x00,
            0x00, 0x01,
        ] as &[u8])
        .unwrap();
        assert_eq!(
            ip,
            IpAddress(net::IpAddr::V6("::ffff:a00:1".parse().unwrap()))
        );
        let ip: IpAddress = deserialize(&[
            0x00u8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x00,
            0x00, 0x01,
        ] as &[u8])
        .unwrap();
