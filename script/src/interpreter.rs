...
use crate::{
    script, Builder, Error, Num, Opcode, Script, ScriptWitness, SignatureChecker, Stack,
    VerificationFlags,
};
    ...
    while pc < script.len() {
        ...
        match opcode {
                ...
                if flags.verify_checkdatasig =>
            {
                ...
                let success =
                    verify_signature(checker, signature.into(), pubkey.into(), message_hash);
                match opcode {
...
mod tests {
    ...
    use crate::{
        Builder, Error, NoopSignatureChecker, Num, Opcode, Script, ScriptWitness, Stack,
        TransactionInputSigner, TransactionSignatureChecker, VerificationFlags,
    };
    use bitcrypto::sha256;
    use chain::Transaction;
    use keys::{KeyPair, Message, Network, Private};
    use primitives::bytes::Bytes;
