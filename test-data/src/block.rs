...
use chain;
use primitives::{bytes::Bytes, compact::Compact, hash::H256};
use script::{Builder as ScriptBuilder, Opcode};
use serialization::{serialized_list_size, Serializable};
use std::cell::Cell;
    ...
    F: Invoke<chain::Transaction>,
{
    ...
    pub fn input_with_size(self, size: usize) -> TransactionInputBuilder<Self> {
        ...
        assert!(
            size >= raw_input_size + script_len_size,
            "Desired input size is too small"
        );
        TransactionInputBuilder::with_callback(self)
            .signature_with_size(size - raw_input_size - script_len_size)
    }
