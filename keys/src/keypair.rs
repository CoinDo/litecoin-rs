...
impl fmt::Display for KeyPair {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        r#try!(writeln!(f, "private: {}", self.private));
        writeln!(f, "public: {}", self.public)
    }
}
...
impl KeyPair {
    ...
    pub fn from_private(private: Private) -> Result<KeyPair, Error> {
        ...
        let s: key::SecretKey = r#try!(key::SecretKey::from_slice(context, &*private.secret));
        let pub_key = r#try!(key::PublicKey::from_secret_key(context, &s));
