use crate::ethcore_rpc::v1::impls::{
    blockchain::{BlockChainClient, BlockChainClientCore},
    miner::{MinerClient, MinerClientCore},
    network::{NetworkClient, NetworkClientCore},
    raw::{RawClient, RawClientCore},
};
use crate::ethcore_rpc::v1::traits::{
    blockchain::BlockChain, miner::Miner, network::Network, raw::Raw,
};
    ...
    for api in apis.list_apis() {
        match api {
            Api::Raw => handler.extend_with(
                RawClient::new(RawClientCore::new(deps.local_sync_node.clone())).to_delegate(),
            ),
