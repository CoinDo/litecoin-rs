...
pub fn manage_unknown_orphaned_blocks(
    ...
    let unknown_to_remove: Vec<H256> = orphaned_blocks_pool
        .remove_blocks(&unknown_to_remove)
        .into_iter()
        .map(|b| b.header.hash)
        .collect();
...
mod tests {
    ...
    fn manage_bad_peers() {
        ...
        use std::time::Duration;
        let config = ManagePeersConfig {
            new_block_failure_interval_ms: 0,
            ..Default::default()
        };
        let mut peers = PeersTasks::default();
    ...
    fn manage_orphan_transactions_by_time() {
        ...
        assert_eq!(
            manage_orphaned_transactions(&config, &mut pool),
