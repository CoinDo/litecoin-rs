...
use crate::synchronization_executor::{Task as SynchronizationTask, TaskExecutor};
    ...
    V: Client,
{
    ...
    pub fn on_getheaders(&self, peer_index: PeerIndex, message: types::GetHeaders, id: RequestId) {
        if self.state.synchronizing() {
            ...
            self.executor
                .execute(SynchronizationTask::Ignore(peer_index, id));
            return;
        }
...
pub mod tests {
    ...
    use crate::synchronization_client_core::{
        Config, CoreVerificationSink, SynchronizationClientCore,
    };
    ...
    fn create_local_node(
        ...
        LocalNode<
            DummyTaskExecutor,
            DummyServer,
            SynchronizationClient<DummyTaskExecutor, DummyVerifier>,
        >,
    ) {
