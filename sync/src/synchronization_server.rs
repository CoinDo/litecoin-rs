    ...
    fn serve_reversed_get_data(
        ...
        match next_item.inv_type {
            ...
            common::InventoryType::MessageFilteredBlock => {
                if let Some(block) = self.storage.block(next_item.hash.clone().into()) {
                    let message_artefacts =
                        self.peers.build_merkle_block(peer_index, &block.into());
                    if let Some(message_artefacts) = message_artefacts {
                        ...
                        self.executor
                            .execute(Task::MerkleBlock(peer_index, message_artefacts.merkleblock));
            ...
            common::InventoryType::MessageCompactBlock => {
                if let Some(block) = self.storage.block(next_item.hash.clone().into()) {
                    ...
                    if let Some(message) = message {
                        ...
                        self.executor
                            .execute(Task::CompactBlock(peer_index, message));
                    }
                } else {
                    notfound.inventory.push(next_item);
                }
...
pub mod tests {
    ...
    fn server_mempool_responds_inventory_when_non_empty_memory_pool() {
        ...
        memory_pool
            .write()
            .insert_verified(transaction.into(), &NonZeroFeeCalculator);
        // when asking for memory pool transactions ids
        server.execute(ServerTask::Mempool(0));
    ...
    fn server_get_block_txn_responds_when_good_request() {
        ...
        server.execute(ServerTask::GetBlockTxn(
            0,
            types::GetBlockTxn {
    ...
    fn server_getdata_responds_transaction_when_transaction_is_in_memory() {
        ...
        // given in-memory transaction
        {
            memory_pool
                .write()
                .insert_verified(tx_verified.clone().into(), &NonZeroFeeCalculator);
        }
        ...
        assert_eq!(tasks, vec![Task::Transaction(0, tx_verified.into()),]);
    }
    ...
    fn server_serves_merkleblock() {
        ...
        let executor = ServerTaskExecutor::new(
            peers.clone(),
            storage.clone(),
            memory_pool.clone(),
            sync_executor.clone(),
        ...
        peers.insert(
            peer_index2,
            Services::default(),
            DummyOutboundSyncConnection::new(),
        );

        let mut loop_task = ServerTask::GetData(
            peer_index2,
            types::GetData {
                inventory: vec![
                    InventoryVector {
                        inv_type: InventoryType::MessageFilteredBlock,
                        hash: b1_hash.clone(),
                    },
                    InventoryVector {
                        inv_type: InventoryType::MessageFilteredBlock,
        ...
        assert_eq!(
            sync_executor.take_tasks(),
            vec![Task::NotFound(
                peer_index2,
                types::NotFound::with_inventory(vec![
                    InventoryVector {
                        inv_type: InventoryType::MessageFilteredBlock,
                        hash: b1_hash.clone()
                    },
        ...
        let mut loop_task = ServerTask::GetData(
            ...
            types::GetData {
                inventory: vec![InventoryVector {
                    inv_type: InventoryType::MessageCompactBlock,
                    hash: b1_hash.clone(),
                }],
            },
        );
