...
impl PeersTasks {
    ...
    pub fn information(&self) -> Information {
        ...
        Information {
            ...
            active: active_for_headers
                .union(&self.blocks_requests.keys().cloned().collect())
                .count(),
        }
    }
    ...
    pub fn on_block_received(&mut self, peer_index: PeerIndex, block_hash: &H256) {
        ...
        let is_last_requested_block_received =
            if let Some(blocks_request) = self.blocks_requests.get_mut(&peer_index) {
        ...
        if !is_last_requested_block_received {
            let mut peer_blocks_requests = self
                .blocks_requests
                .remove(&peer_index)
                .expect("checked above; qed");
            peer_blocks_requests.timestamp = precise_time_s();
            self.blocks_requests
                .insert(peer_index, peer_blocks_requests);
            return;
        }
    ...
    pub fn on_blocks_requested(&mut self, peer_index: PeerIndex, blocks_hashes: &[H256]) {
        ...
        if !self.blocks_requests.contains_key(&peer_index) {
            self.blocks_requests
                .insert(peer_index, BlocksRequest::new());
        }
        self.blocks_requests
            .get_mut(&peer_index)
    ...
    pub fn on_headers_requested(&mut self, peer_index: PeerIndex) {
        ...
        self.headers_requests.remove(&peer_index);
        self.headers_requests
            .insert(peer_index, HeadersRequest::new());
    }
        ...
        for hash in hashes {
            let is_failed_block = {
                let block_stats = self
                    .blocks_stats
                    .entry(hash.clone())
                    .or_insert_with(BlockStats::default);
...
impl fmt::Debug for Information {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{} (act: {}, idl: {}, bad: {})",
            self.all, self.active, self.idle, self.unuseful
        )
    }
}
