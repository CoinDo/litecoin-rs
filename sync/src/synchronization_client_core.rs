...
use chain::{IndexedBlock, IndexedBlockHeader, IndexedTransaction, Transaction};
use futures::Future;
use log::*;
...
pub mod tests {
    ...
    fn synchronization_in_order_block_path_nearly_saturated() {
        ...
        sync.on_headers(
            5,
            types::Headers::with_headers(vec![block1.block_header.clone()]),
        );
        let tasks = executor.take_tasks();
        assert_eq!(
            tasks,
    ...
    fn synchronization_out_of_order_block_path() {
        ...
        sync.on_headers(
            ...
            types::Headers::with_headers(vec![
                test_data::block_h1().block_header.clone(),
                test_data::block_h2().block_header.clone(),
            ]),
        );
    ...
    fn synchronization_parallel_peers() {
...

        {
            ...
            sync.on_headers(
                2,
                types::Headers::with_headers(vec![
                    block1.block_header.clone(),
                    block2.block_header.clone(),
                ]),
            );
            assert_eq!(core.lock().information().chain.requested, 2);
            // synchronization has started && new blocks have been requested
    ...
    fn synchronization_reset_when_peer_is_disconnected() {
        ...
        // request new blocks
        {
            sync.on_headers(
                1,
                types::Headers::with_headers(vec![test_data::block_h1().block_header]),
            );
            assert!(core.lock().information().state.is_nearly_saturated());
        }
    ...
    fn synchronization_not_starting_when_receiving_known_blocks() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![test_data::genesis().block_header]),
        );
    ...
    fn synchronization_asks_for_inventory_after_saturating() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![block.block_header.clone()]),
        );
        sync.on_headers(
            2,
            types::Headers::with_headers(vec![block.block_header.clone()]),
    ...
    fn synchronization_remembers_correct_block_headers_in_order() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![b1.block_header.clone(), b2.block_header.clone()]),
        );
    ...
    fn synchronization_works_for_forks_from_db_best_block() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![
                fork1[0].block_header.clone(),
                fork1[1].block_header.clone(),
            ]),
        );
        sync.on_headers(
            2,
            types::Headers::with_headers(vec![
                fork2[0].block_header.clone(),
                fork2[1].block_header.clone(),
                fork2[2].block_header.clone(),
            ]),
        );
        let tasks = { executor.take_tasks() };
        assert_eq!(
    ...
    fn synchronization_works_for_forks_long_after_short() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![
                common_block.block_header.clone(),
                fork1[0].block_header.clone(),
                fork1[1].block_header.clone(),
            ]),
        );
    ...
    fn transaction_is_not_requested_when_synchronizing() {
        ...
        sync.on_inventory(
            0,
            types::Inv::with_inventory(vec![InventoryVector::tx(H256::from(0))]),
        );
    ...
    fn transaction_is_requested_when_not_synchronizing() {
        ...
        sync.on_inventory(
            0,
            types::Inv::with_inventory(vec![InventoryVector::witness_tx(H256::from(0))]),
        );

        {
            ...
            assert_eq!(
                ...
                vec![Task::GetData(
                    0,
                    types::GetData::with_inventory(vec![InventoryVector::witness_tx(H256::from(
                        0
                    ))])
                )]
            );
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![b1.block_header.clone()]),
        );
        ...
        sync.on_inventory(
            0,
            types::Inv::with_inventory(vec![InventoryVector::witness_tx(H256::from(1))]),
        );
    ...
    fn when_previous_block_verification_failed_fork_is_not_requested() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![
                b10.block_header.clone(),
                b11.block_header.clone(),
                b12.block_header.clone(),
            ]),
        );
        sync.on_headers(
            2,
            types::Headers::with_headers(vec![
                b10.block_header.clone(),
                b21.block_header.clone(),
                b22.block_header.clone(),
            ]),
        );
    ...
    fn relay_new_block_when_in_saturated_state() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![b0.block_header.clone(), b1.block_header.clone()]),
        );
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![b3.block_header.clone()]),
        );
    ...
    fn when_peer_does_not_respond_to_block_requests() {
        ...
        sync.on_headers(
            1,
            types::Headers::with_headers(vec![b0.block_header.clone()]),
        );
    ...
    fn when_transaction_double_spends_during_reorg() {
        ...
        assert!(mempool
            .write()
            .remove_by_hash(&b1.transactions[2].hash())
            .is_some());
    ...
    fn sync_listener_calls() {
        ...
        sync.on_headers(
            ...
            types::Headers::with_headers(vec![
                ...
                test_data::block_h3().block_header,
            ]),
        );
