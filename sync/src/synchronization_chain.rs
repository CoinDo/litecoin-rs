...
impl Chain {
    ...
    pub fn block_header_by_number(&self, number: BlockHeight) -> Option<IndexedBlockHeader> {
        if number <= self.best_storage_block.number {
            self.storage
                .block_header(storage::BlockRef::Number(number))
                .map(Into::into)
        } else {
            self.headers_chain
        ...
        match block_origin {
            ...
            storage::BlockOrigin::SideChainBecomingCanonChain(origin) => {
                ...
                let old_main_blocks_transactions = origin
                    ...
                    .into_iter()
                    .flat_map(|block_hash| {
                        self.storage.indexed_block_transactions(block_hash.into())
                    })
                    .collect::<Vec<_>>();
                ...
                let remove_verifying_transaction = {
                    if let Some(entry) = self.verifying_transactions.get(&h) {
                        if entry
                            ...
                            .inputs
                            .iter()
                            .any(|i| i.previous_output.hash == hash)
                        {
...
mod tests {
    ...
    fn chain_transaction_state() {
        ...
        let tx2: Transaction =
            test_data::TransactionBuilder::with_input(&test_data::genesis().transactions[0], 0)
                .into();
        ...
        assert_eq!(
            chain.transaction_state(&block2.transactions[0].hash()),
            TransactionState::Unknown
        );
        assert_eq!(
            chain.transaction_state(&tx1_hash),
            TransactionState::Verifying
        );
        assert_eq!(
            chain.transaction_state(&tx2_hash),
            TransactionState::InMemory
        );
    }
    ...
    fn memory_pool_transactions_are_reverified_after_reorganization() {
        ...
        assert!(result
            ...
            .iter()
            .any(|ref tx| &tx.hash == &tx1_hash));
        assert!(result
            .transactions_to_reverify
            .iter()
            .any(|ref tx| &tx.hash == &tx2_hash));
    }
    ...
    fn fork_chain_block_transaction_is_removed_from_on_block_insert() {
        ...
        assert_eq!(
            chain
                .insert_best_block(b0.clone().into())
        ...
        assert_eq!(
            chain
                .insert_best_block(b1.clone().into())
                .expect("block accepted"),
        ...
        assert_eq!(
            chain
                .insert_best_block(b2.clone().into())
                .expect("block accepted"),
        ...
        assert_eq!(
            chain
                .insert_best_block(b3.clone().into())
                .expect("block accepted"),
        ...
        assert_eq!(
            chain
                .insert_best_block(b4.clone().into())
                .expect("block accepted"),
            BlockInsertionResult::default()
        );
        assert_eq!(chain.information().transactions.transactions_count, 3);
        // order matters
        let insert_result = chain
            .insert_best_block(b5.clone().into())
            .expect("block accepted");
