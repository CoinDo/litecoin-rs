...
impl<'a> BlockVerifier<'a> {
    ...
    pub fn check(&self) -> Result<(), Error> {
        ...
        r#try!(self.coinbase.check());
        r#try!(self.serialized_size.check());
        r#try!(self.extra_coinbases.check());
        r#try!(self.transactions_uniqueness.check());
        r#try!(self.sigops.check());
        r#try!(self.merkle_root.check());
        Ok(())
    }
}
