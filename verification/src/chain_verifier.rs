...
use chain::{BlockHeader, IndexedBlock, IndexedBlockHeader, Transaction};
use log::*;
use network::ConsensusParams;
use primitives::hash::H256;
use storage::{
    duplex_store::{DuplexTransactionOutputProvider, NoopStore},
    BlockHeaderProvider, BlockOrigin, SharedStore, TransactionOutputProvider,
};
...
impl BackwardsCompatibleChainVerifier {
        ...
        T: TransactionOutputProvider,
    {
        ...
        let tx_verifier =
            MemoryPoolTransactionVerifier::new(&indexed_tx, &self.consensus, &deployments);
...
mod tests {
    ...
    use crate::{
        constants::DOUBLE_SPACING_SECONDS, error::Error, error::TransactionError,
    ...
    fn transaction_references_same_block_and_goes_before_previous() {
        ...
        assert_eq!(
            expected,
            verifier.verify(VerificationLevel::Header, &block.clone().into())
        );
        ...
        let canonical_consensus = ConsensusParams::new(
            Network::Unitest,
            ConsensusFork::BitcoinCash(canonical_params),
        );
        let verifier = ChainVerifier::new(storage, canonical_consensus);
        let expected = Ok(());
        assert_eq!(
            expected,
            verifier.verify(VerificationLevel::Header, &block.into())
        );
    }
