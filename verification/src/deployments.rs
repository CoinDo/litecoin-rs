...
impl<'a> BlockDeployments<'a> {
    ...
    pub fn csv(&self) -> bool {
        self.deployments
            .csv(self.number, self.headers, self.consensus)
    }

    pub fn segwit(&self) -> bool {
        self.deployments
            .segwit(self.number, self.headers, self.consensus)
    }
}
...
fn threshold_state(
    ...
    match cache.entry(deployment.name) {
        ...
        Entry::Occupied(ref entry)
            if entry.get().block_number == number && entry.get().block_hash == hash =>
        {
            entry.get().state
        }
        ...
        Entry::Occupied(mut entry) => {
            ...
            let state = deployment_iter
                .last()
                .expect("iter must have at least one item");
...
fn count_deployment_matches(
    block_number: u32,
    blocks: &BlockHeaderProvider,
    deployment: Deployment,
    window: u32,
) -> usize {
        ...
        match self.last_state {
            ...
            ThresholdState::Started => {
                if median >= self.deployment.timeout {
                    ...
                    let count = count_deployment_matches(
                        block_number,
                        self.headers,
                        self.deployment,
...
mod tests {
    ...
    impl DeploymentHeaderProvider {
        pub fn mine(&mut self, height: u32, time: u32, version: u32) {
            ...
            while self.by_height.len() < height as usize {
                ...
                self.by_hash
                    .insert(previous_header_hash.clone(), self.by_height.len() - 1);
            }
        }
    }
    ...
    fn test_threshold_state_defined_to_failed() {
        ...
        let test_cases = vec![
            ...
            (
                999,
                make_test_time(20000),
                0x20000001,
                ThresholdState::Defined,
            ),
            (
                1000,
                make_test_time(30000),
                0x20000001,
                ThresholdState::Failed,
            ),
            (
                1999,
                make_test_time(30001),
                0x20000001,
                ThresholdState::Failed,
            ),
            (
                2000,
                make_test_time(30002),
                0x20000001,
                ThresholdState::Failed,
            ),
            (
                2001,
                make_test_time(30003),
                0x20000001,
                ThresholdState::Failed,
            ),
            (
                2999,
                make_test_time(30004),
                0x20000001,
                ThresholdState::Failed,
            ),
            (
                3000,
                make_test_time(30005),
                0x20000001,
                ThresholdState::Failed,
            ),
        ];
    ...
    fn test_threshold_state_defined_to_started_to_failed() {
        ...
        let test_cases = vec![
            ...
            (
                1000,
                make_test_time(10000) - 1,
                0x20000001,
                ThresholdState::Defined,
            ),
            (
                2000,
                make_test_time(10000),
                0x20000001,
                ThresholdState::Started,
            ),
            (
                2051,
                make_test_time(10010),
                0x20000000,
                ThresholdState::Started,
            ),
            (
                2950,
                make_test_time(10020),
                0x20000001,
                ThresholdState::Started,
            ),
            (
