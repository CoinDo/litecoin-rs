...
use std::ops;
use storage::duplex_store::NoopStore;
...
impl<'a> TransactionVerifier<'a> {
    ...
    pub fn check(&self) -> Result<(), TransactionError> {
        r#try!(self.empty.check());
        r#try!(self.null_non_coinbase.check());
        r#try!(self.oversized_coinbase.check());
        Ok(())
    }
}
    ...
    pub fn new(
        ...
        MemoryPoolTransactionVerifier {
            ...
            sigops: TransactionSigops::new(
                transaction,
                ConsensusFork::absolute_maximum_block_sigops(),
            ),
        }
    }
