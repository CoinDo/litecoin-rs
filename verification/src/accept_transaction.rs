...
use storage::{
    duplex_store::{transaction_index_for_output_check, DuplexTransactionOutputProvider},
...
impl<'a> TransactionAcceptor<'a> {
    ...
    pub fn check(&self) -> Result<(), TransactionError> {
        r#try!(self.size.check());
        r#try!(self.premature_witness.check());
        r#try!(self.bip30.check());
        r#try!(self.missing_inputs.check());
        r#try!(self.maturity.check());
        r#try!(self.overspent.check());
        r#try!(self.double_spent.check());
        r#try!(self.return_replay_protection.check());
        r#try!(self.eval.check());
        Ok(())
    }
}
...
mod tests {
    ...
    use crate::{canon::CanonTransaction, error::TransactionError};
