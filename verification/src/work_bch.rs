...
use crate::work::{is_retarget_height, work_required_retarget, work_required_testnet};
...
use crate::constants::{DOUBLE_SPACING_SECONDS, RETARGETING_INTERVAL, TARGET_SPACING_SECONDS};

/// Returns work required for given header for the post-HF Bitcoin Cash block
pub fn work_required_bitcoin_cash(
    parent_header: IndexedBlockHeader,
    time: u32,
    height: u32,
    store: &BlockHeaderProvider,
    consensus: &ConsensusParams,
    fork: &BitcoinCashConsensusParams,
    max_bits: Compact,
) -> Compact {
    ...
    let timestamp_diff = parent_timestamp
        .checked_sub(ancient_timestamp)
        .unwrap_or_default();
...
fn work_required_bitcoin_cash_adjusted(
    ...
    fn suitable_block(mut header2: BlockHeader, store: &BlockHeaderProvider) -> BlockHeader {
        ...
        let mut header1 = store
            .block_header(header2.previous_header_hash.clone().into())
            .expect(reason);
        let mut header0 = store
            .block_header(header1.previous_header_hash.clone().into())
            .expect(reason);
    ...
    let first_header = store
        .block_header(first_height.into())
...
mod tests {
    ...
    fn bitcoin_cash_req7() {
        ...
        let uahf_consensus = ConsensusParams::new(
            Network::Mainnet,
            ConsensusFork::BitcoinCash(BitcoinCashConsensusParams {
                height: 1000,
        ...
        for height in 1000..1010 {
            ...
            let main_bits: u32 = work_required(
                header.hash(),
                0,
                height as u32,
                &header_provider,
            ...
            let uahf_bits: u32 = work_required(
                header.hash(),
                0,
                height as u32,
                &header_provider,
        ...
        let main_bits: u32 =
            work_required(header.hash(), 0, 1010, &header_provider, &main_consensus).into();
        assert_eq!(main_bits, 0x207fffff_u32);
        let uahf_bits: u32 =
            work_required(header.hash(), 0, 1010, &header_provider, &uahf_consensus).into();
    ...
    fn bitcoin_cash_adjusted_difficulty() {
        ...
        let current_bits = work_required_bitcoin_cash_adjusted(
            header_provider.block_header(2049.into()).unwrap().into(),
            0,
            2050,
            &header_provider,
            &uahf_consensus,
        ...
        for height in 2050..2060 {
            ...
            let calculated_bits = work_required_bitcoin_cash_adjusted(
                header_provider.block_header(height.into()).unwrap().into(),
                0,
                height + 1,
                &header_provider,
                &uahf_consensus,
        ...
        header_provider.insert(header);
        let calculated_bits = work_required_bitcoin_cash_adjusted(
            header_provider.block_header(2060.into()).unwrap().into(),
            0,
            2061,
            &header_provider,
            &uahf_consensus,
        ...
        header_provider.insert(header);
        let calculated_bits = work_required_bitcoin_cash_adjusted(
            header_provider.block_header(2061.into()).unwrap().into(),
            0,
            2062,
            &header_provider,
            &uahf_consensus,
        ...
        header_provider.insert(header);
        let calculated_bits = work_required_bitcoin_cash_adjusted(
            header_provider.block_header(2082.into()).unwrap().into(),
            0,
            2083,
            &header_provider,
            &uahf_consensus,
        ...
        header.bits = current_bits;
        header_provider.insert(header);
        let mut current_bits = work_required_bitcoin_cash_adjusted(
            header_provider.block_header(2113.into()).unwrap().into(),
            0,
            2114,
            &header_provider,
            &uahf_consensus,
        ...
        header_provider.insert(header);
        let mut current_bits = work_required_bitcoin_cash_adjusted(
            header_provider.block_header(2207.into()).unwrap().into(),
            0,
            2208,
            &header_provider,
            &uahf_consensus,
