use crate::constants::{
    DOUBLE_SPACING_SECONDS, MAX_TIMESPAN, MIN_TIMESPAN, RETARGETING_INTERVAL,
    TARGET_TIMESPAN_SECONDS,
};
use crate::work_bch::work_required_bitcoin_cash;
use bigint::U256;
use chain::{BlockHeader, IndexedBlockHeader};
use network::{ConsensusFork, ConsensusParams, Network};
use primitives::{compact::Compact, hash::H256};
use std::cmp;
...
pub fn work_required(
    ...
    match consensus.fork {
        ConsensusFork::BitcoinCash(ref fork) if height >= fork.height => {
            return work_required_bitcoin_cash(
                IndexedBlockHeader {
                    hash: parent_hash,
                    raw: parent_header,
                },
